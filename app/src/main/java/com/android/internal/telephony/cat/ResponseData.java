...
class DTTZResponseData extends ResponseData {
    ...
    public void format(ByteArrayOutputStream buf) {
        ...
        // Fill year byte
        data[1] = byteToBCD(mCalendar.get(Calendar.YEAR) % 100);

        // Fill month byte
        data[2] = byteToBCD(mCalendar.get(Calendar.MONTH) + 1);

        // Fill day byte
        data[3] = byteToBCD(mCalendar.get(Calendar.DATE));

        // Fill hour byte
        data[4] = byteToBCD(mCalendar.get(Calendar.HOUR_OF_DAY));

        // Fill minute byte
        data[5] = byteToBCD(mCalendar.get(Calendar.MINUTE));

        // Fill second byte
        data[6] = byteToBCD(mCalendar.get(Calendar.SECOND));
