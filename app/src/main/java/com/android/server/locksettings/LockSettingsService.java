...
public class LockSettingsService extends ILockSettings.Stub {
        ...
        try {
            ...
            if (getString("migrated_user_specific", null, 0) == null) {
                ...
                for (int user = 0; user < users.size(); user++) {
                    ...
                    String ownerInfo = Secure.getStringForUser(cr, OWNER_INFO, userId);
                    if (!TextUtils.isEmpty(ownerInfo)) {
                        setString(OWNER_INFO, ownerInfo, userId);
                        Secure.putStringForUser(cr, OWNER_INFO, "", userId);
                    }
                    ...
                    boolean enabled;
                    try {
                        int ivalue = Secure.getIntForUser(cr, OWNER_INFO_ENABLED, userId);
                        enabled = ivalue != 0;
                    ...
                    Secure.putIntForUser(cr, OWNER_INFO_ENABLED, 0, userId);
                }
                // No need to move the password / pattern files. They're already in the right place.
    ...
    public String getStringUnchecked(String key, String defaultValue, int userId) {
        if (Secure.LOCK_PATTERN_ENABLED.equals(key)) {
        ...
        if (LockPatternUtils.LEGACY_LOCK_PATTERN_ENABLED.equals(key)) {
            key = Secure.LOCK_PATTERN_ENABLED;
        }
