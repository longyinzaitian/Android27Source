...
class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int currentVersion) {
        ...
        if (upgradeVersion == 27) {
            String[] settingsToMove = {
                    ...
                    Secure.LOGGING_ID,
                    Secure.NETWORK_PREFERENCE,
                    Secure.PARENTAL_CONTROL_ENABLED,
                    Secure.PARENTAL_CONTROL_LAST_UPDATE,
                    Secure.PARENTAL_CONTROL_REDIRECT_URL,
                    Secure.SETTINGS_CLASSNAME,
                    Secure.USB_MASS_STORAGE_ENABLED,
                    Secure.USE_GOOGLE_MAIL,
                    Secure.WIFI_NETWORKS_AVAILABLE_NOTIFICATION_ON,
                    Secure.WIFI_NETWORKS_AVAILABLE_REPEAT_DELAY,
                    Secure.WIFI_NUM_OPEN_NETWORKS_KEPT,
                    Secure.WIFI_ON,
                    ...
                    Secure.WIFI_WATCHDOG_MAX_AP_CHECKS,
                    Secure.WIFI_WATCHDOG_ON,
                    Secure.WIFI_WATCHDOG_PING_COUNT,
                    Secure.WIFI_WATCHDOG_PING_DELAY_MS,
        ...
        if (upgradeVersion == 38) {
            ...
            try {
                ...
                db.execSQL("INSERT OR IGNORE INTO secure(name,value) values('" +
                        Global.ASSISTED_GPS_ENABLED + "','" + value + "');");
                db.setTransactionSuccessful();
            } finally {
                db.endTransaction();
            }
        ...
        if (upgradeVersion == 45) {
            ...
            try {
                db.execSQL("INSERT INTO secure(name,value) values('" +
                        Secure.MOUNT_PLAY_NOTIFICATION_SND + "','1');");
                db.execSQL("INSERT INTO secure(name,value) values('" +
                        Secure.MOUNT_UMS_AUTOSTART + "','0');");
                db.execSQL("INSERT INTO secure(name,value) values('" +
                        Secure.MOUNT_UMS_PROMPT + "','1');");
                db.execSQL("INSERT INTO secure(name,value) values('" +
                        Secure.MOUNT_UMS_NOTIFY_ENABLED + "','1');");
                db.setTransactionSuccessful();
            } finally {
                db.endTransaction();
            }
       ...
       if (upgradeVersion == 51) {
        ...
        if (upgradeVersion == 64) {
            ...
            try {
                stmt = db.compileStatement("INSERT INTO secure(name,value)"
                        + " VALUES(?,?);");
                loadIntegerSetting(stmt, Secure.LONG_PRESS_TIMEOUT,
        ...
        if (upgradeVersion == 67) {
            ...
            try {
                stmt = db.compileStatement("INSERT INTO secure(name,value)"
                        + " VALUES(?,?);");
                loadBooleanSetting(stmt, Secure.TOUCH_EXPLORATION_ENABLED,
        ...
        if (upgradeVersion == 71) {
            ...
            try {
                stmt = db.compileStatement("INSERT INTO secure(name,value)"
                        + " VALUES(?,?);");
                loadBooleanSetting(stmt, Secure.ACCESSIBILITY_SPEAK_PASSWORD,
        ...
        if (upgradeVersion == 80) {
            ...
            try {
                stmt = db.compileStatement("INSERT OR REPLACE INTO secure(name,value)"
                        + " VALUES(?,?);");
                loadBooleanSetting(stmt, Secure.SCREENSAVER_ENABLED,
                        com.android.internal.R.bool.config_dreamsEnabledByDefault);
                loadBooleanSetting(stmt, Secure.SCREENSAVER_ACTIVATE_ON_DOCK,
                        com.android.internal.R.bool.config_dreamsActivatedOnDockByDefault);
                loadBooleanSetting(stmt, Secure.SCREENSAVER_ACTIVATE_ON_SLEEP,
                        com.android.internal.R.bool.config_dreamsActivatedOnSleepByDefault);
                loadStringSetting(stmt, Secure.SCREENSAVER_COMPONENTS,
                        com.android.internal.R.string.config_dreamsDefaultComponent);
                loadStringSetting(stmt, Secure.SCREENSAVER_DEFAULT_COMPONENT,
                        com.android.internal.R.string.config_dreamsDefaultComponent);
        ...
        if (upgradeVersion == 81) {
            ...
            try {
                stmt = db.compileStatement("INSERT OR REPLACE INTO secure(name,value)"
                        + " VALUES(?,?);");
                loadBooleanSetting(stmt, Global.PACKAGE_VERIFIER_ENABLE,
        ...
        if (upgradeVersion == 83) {
            ...
            try {
                ...
                stmt.close();
                stmt = db.compileStatement("INSERT INTO secure(name,value) VALUES(?,?);");
                loadFractionSetting(stmt, Secure.ACCESSIBILITY_DISPLAY_MAGNIFICATION_SCALE,
            ...
            if (mUserHandle == UserHandle.USER_SYSTEM) {
                ...
                try {
                    String[] settingsToMove = {
                            ...
                            Global.PACKAGE_VERIFIER_TIMEOUT,
                            Global.PACKAGE_VERIFIER_DEFAULT_RESPONSE
                    };
                    moveSettingsToNewTable(db, TABLE_SECURE, TABLE_GLOBAL, settingsToMove, true);
        ...
        if (upgradeVersion == 88) {
            if (mUserHandle == UserHandle.USER_SYSTEM) {
                ...
                try {
                    String[] settingsToMove = {
                            Global.BATTERY_DISCHARGE_DURATION_THRESHOLD,
                            Global.BATTERY_DISCHARGE_THRESHOLD,
                            Global.SEND_ACTION_APP_ERROR,
                            Global.DROPBOX_AGE_SECONDS,
                            Global.DROPBOX_MAX_FILES,
                            Global.DROPBOX_QUOTA_KB,
                            Global.DROPBOX_QUOTA_PERCENT,
                            Global.DROPBOX_RESERVE_PERCENT,
                            Global.DROPBOX_TAG_PREFIX,
                            Global.ERROR_LOGCAT_PREFIX,
                            Global.SYS_FREE_STORAGE_LOG_INTERVAL,
                            Global.DISK_FREE_CHANGE_REPORTING_THRESHOLD,
                            Global.SYS_STORAGE_THRESHOLD_PERCENTAGE,
                            Global.SYS_STORAGE_THRESHOLD_MAX_BYTES,
                            Global.SYS_STORAGE_FULL_THRESHOLD_BYTES,
                            Global.SYNC_MAX_RETRY_DELAY_IN_SECONDS,
                            Global.CONNECTIVITY_CHANGE_DELAY,
                            Global.CAPTIVE_PORTAL_DETECTION_ENABLED,
                            Global.CAPTIVE_PORTAL_SERVER,
                            Global.NSD_ON,
                            Global.SET_INSTALL_LOCATION,
                            Global.DEFAULT_INSTALL_LOCATION,
                            Global.INET_CONDITION_DEBOUNCE_UP_DELAY,
                            Global.INET_CONDITION_DEBOUNCE_DOWN_DELAY,
                            Global.READ_EXTERNAL_STORAGE_ENFORCED_DEFAULT,
                            Global.HTTP_PROXY,
                            Global.GLOBAL_HTTP_PROXY_HOST,
                            Global.GLOBAL_HTTP_PROXY_PORT,
                            Global.GLOBAL_HTTP_PROXY_EXCLUSION_LIST,
            ...
            if (mUserHandle == UserHandle.USER_SYSTEM) {
                ...
                try {
                    String[] prefixesToMove = {
                            ...
                            Global.BLUETOOTH_A2DP_SINK_PRIORITY_PREFIX,
                            Global.BLUETOOTH_INPUT_DEVICE_PRIORITY_PREFIX,
                    };
        ...
        if (upgradeVersion == 91) {
            if (mUserHandle == UserHandle.USER_SYSTEM) {
                ...
                try {
                    ...
                    String[] settingsToMove = { Global.MODE_RINGER };
                    moveSettingsToNewTable(db, TABLE_SYSTEM, TABLE_GLOBAL, settingsToMove, true);
        ...
        if (upgradeVersion == 92) {
            ...
            try {
                ...
                if (mUserHandle == UserHandle.USER_SYSTEM) {
                    ...
                    loadSetting(stmt, Secure.USER_SETUP_COMPLETE,
                            deviceProvisioned);
                } else {
                    // otherwise use the default
                    loadBooleanSetting(stmt, Secure.USER_SETUP_COMPLETE,
            ...
            if (mUserHandle == UserHandle.USER_SYSTEM) {
                ...
                try {
                    stmt = db.compileStatement("INSERT OR REPLACE INTO global(name,value)"
                            + " VALUES(?,?);");
                    loadStringSetting(stmt, Global.WIRELESS_CHARGING_STARTED_SOUND,
        ...
        if (upgradeVersion == 95) {
            if (mUserHandle == UserHandle.USER_SYSTEM) {
                db.beginTransaction();
                try {
                    String[] settingsToMove = { Global.BUGREPORT_IN_POWER_MENU };
                    moveSettingsToNewTable(db, TABLE_SECURE, TABLE_GLOBAL, settingsToMove, true);
                    db.setTransactionSuccessful();
                } finally {
                    db.endTransaction();
        ...
        if (upgradeVersion == 97) {
            if (mUserHandle == UserHandle.USER_SYSTEM) {
                ...
                try {
                    stmt = db.compileStatement("INSERT OR REPLACE INTO global(name,value)"
                            + " VALUES(?,?);");
                    loadIntegerSetting(stmt, Global.LOW_BATTERY_SOUND_TIMEOUT,
            ...
            if (mUserHandle == UserHandle.USER_SYSTEM) {
                ...
                try {
                    stmt = db.compileStatement("INSERT OR IGNORE INTO global(name,value)"
                            + " VALUES(?,?);");
                    loadSetting(stmt, Global.DEVICE_NAME, getDefaultDeviceName());
                    db.setTransactionSuccessful();
                } finally {
                    db.endTransaction();
                    if (stmt != null) stmt.close();
        ...
        if (upgradeVersion == 102) {
            ...
            try {
                ...
                if (mUserHandle == UserHandle.USER_SYSTEM) {
                    ...
                    stmt = db.compileStatement("INSERT OR IGNORE INTO secure(name,value)"
                            + " VALUES(?,?);");
                    loadBooleanSetting(stmt, Secure.INSTALL_NON_MARKET_APPS,
        ...
        if (upgradeVersion == 103) {
            ...
            try {
                stmt = db.compileStatement("INSERT OR REPLACE INTO secure(name,value)"
                        + " VALUES(?,?);");
                loadBooleanSetting(stmt, Secure.WAKE_GESTURE_ENABLED,
            ...
            try {
                ...
                if (mUserHandle == UserHandle.USER_SYSTEM) {
                    final int oldShow = getIntValueFromTable(db,
                            TABLE_GLOBAL, Secure.LOCK_SCREEN_SHOW_NOTIFICATIONS, -1);
                    if (oldShow >= 0) {
                        // overwrite the default with whatever you had
                        loadSetting(stmt, Secure.LOCK_SCREEN_SHOW_NOTIFICATIONS, oldShow);
                        final SQLiteStatement deleteStmt
                                = db.compileStatement("DELETE FROM global WHERE name=?");
                        deleteStmt.bindString(1, Secure.LOCK_SCREEN_SHOW_NOTIFICATIONS);
                        deleteStmt.execute();
                    }
                }
                db.setTransactionSuccessful();
        ...
        if (upgradeVersion < 107) {
            ...
            if (mUserHandle == UserHandle.USER_SYSTEM) {
                ...
                try {
                    stmt = db.compileStatement("INSERT OR REPLACE INTO global(name,value)"
                            + " VALUES(?,?);");
                    loadStringSetting(stmt, Global.TRUSTED_SOUND,
        ...
        if (upgradeVersion < 111) {
            ...
            if (mUserHandle == UserHandle.USER_SYSTEM) {
                ...
                try {
                    stmt = db.compileStatement("INSERT OR REPLACE INTO global(name,value)"
                            + " VALUES(?,?);");
                    loadSetting(stmt, Global.MODE_RINGER, AudioManager.RINGER_MODE_NORMAL);
                    db.setTransactionSuccessful();
                } finally {
                    db.endTransaction();
                    if (stmt != null) stmt.close();
        ...
        if (upgradeVersion < 112) {
            if (mUserHandle == UserHandle.USER_SYSTEM) {
                ...
                try {
                    ...
                    stmt.bindString(2, Global.DEVICE_NAME);
        ...
        if (upgradeVersion < 113) {
            ...
            try {
                stmt = db.compileStatement("INSERT OR IGNORE INTO secure(name,value)"
                        + " VALUES(?,?);");
                loadIntegerSetting(stmt, Secure.SLEEP_TIMEOUT,
        ...
        if (upgradeVersion < 116) {
            if (mUserHandle == UserHandle.USER_SYSTEM) {
                ...
                try {
                    stmt = db.compileStatement("INSERT OR IGNORE INTO global(name,value)"
                            + " VALUES(?,?);");
                    loadSetting(stmt, Global.ENHANCED_4G_MODE_ENABLED,
        ...
        if (upgradeVersion < 117) {
            ...
            try {
                String[] systemToSecure = {
                        Secure.LOCK_TO_APP_EXIT_LOCKED
                };
    ...
    private void loadSecureSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            loadStringSetting(stmt, Secure.LOCATION_PROVIDERS_ALLOWED,
            ...
            loadSetting(stmt, Secure.ALLOW_MOCK_LOCATION,
                    "1".equals(SystemProperties.get("ro.allow.mock.location")) ? 1 : 0);

            loadSecure35Settings(stmt);

            loadBooleanSetting(stmt, Secure.MOUNT_PLAY_NOTIFICATION_SND,
                    R.bool.def_mount_play_notification_snd);

            loadBooleanSetting(stmt, Secure.MOUNT_UMS_AUTOSTART,
                    R.bool.def_mount_ums_autostart);

            loadBooleanSetting(stmt, Secure.MOUNT_UMS_PROMPT,
                    R.bool.def_mount_ums_prompt);

            loadBooleanSetting(stmt, Secure.MOUNT_UMS_NOTIFY_ENABLED,
            ...
            loadBooleanSetting(stmt, Secure.ACCESSIBILITY_DISPLAY_MAGNIFICATION_ENABLED,
                    R.bool.def_accessibility_display_magnification_enabled);
            ...
            loadStringSetting(stmt, Secure.IMMERSIVE_MODE_CONFIRMATIONS,
    ...
    private void loadSecure35Settings(SQLiteStatement stmt) {
        loadBooleanSetting(stmt, Secure.BACKUP_ENABLED,
                R.bool.def_backup_enabled);

        loadStringSetting(stmt, Secure.BACKUP_TRANSPORT,
    ...
    private void loadGlobalSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            loadBooleanSetting(stmt, Global.AIRPLANE_MODE_ON,
                    R.bool.def_airplane_mode_on);

            loadBooleanSetting(stmt, Global.THEATER_MODE_ON,
                    R.bool.def_theater_mode_on);

            loadStringSetting(stmt, Global.AIRPLANE_MODE_RADIOS,
                    R.string.def_airplane_mode_radios);

            loadStringSetting(stmt, Global.AIRPLANE_MODE_TOGGLEABLE_RADIOS,
                    R.string.airplane_mode_toggleable_radios);

            loadBooleanSetting(stmt, Global.ASSISTED_GPS_ENABLED,
                    R.bool.assisted_gps_enabled);

            loadBooleanSetting(stmt, Global.AUTO_TIME,
                    R.bool.def_auto_time); // Sync time to NITZ

            loadBooleanSetting(stmt, Global.AUTO_TIME_ZONE,
                    R.bool.def_auto_time_zone); // Sync timezone to NITZ

            loadSetting(stmt, Global.STAY_ON_WHILE_PLUGGED_IN,
                    ("1".equals(SystemProperties.get("ro.kernel.qemu")) ||
                        mContext.getResources().getBoolean(R.bool.def_stay_on_while_plugged_in))
                     ? 1 : 0);

            loadIntegerSetting(stmt, Global.WIFI_SLEEP_POLICY,
                    R.integer.def_wifi_sleep_policy);

            loadSetting(stmt, Global.MODE_RINGER,
            ...
            loadBooleanSetting(stmt, Global.WIFI_ON,
                    R.bool.def_wifi_on);

            loadBooleanSetting(stmt, Global.WIFI_NETWORKS_AVAILABLE_NOTIFICATION_ON,
                    R.bool.def_networks_available_notification_on);

            loadBooleanSetting(stmt, Global.BLUETOOTH_ON,
                    R.bool.def_bluetooth_on);

            // Enable or disable Cell Broadcast SMS
            loadSetting(stmt, Global.CDMA_CELL_BROADCAST_SMS,
                    RILConstants.CDMA_CELL_BROADCAST_SMS_DISABLED);

            // Data roaming default, based on build
            loadSetting(stmt, Global.DATA_ROAMING,
                    "true".equalsIgnoreCase(
                            SystemProperties.get("ro.com.android.dataroaming",
                                    "false")) ? 1 : 0);

            loadBooleanSetting(stmt, Global.DEVICE_PROVISIONED,
                    R.bool.def_device_provisioned);

            final int maxBytes = mContext.getResources().getInteger(
                    R.integer.def_download_manager_max_bytes_over_mobile);
            if (maxBytes > 0) {
                loadSetting(stmt, Global.DOWNLOAD_MAX_BYTES_OVER_MOBILE,
            ...
            if (recommendedMaxBytes > 0) {
                loadSetting(stmt, Global.DOWNLOAD_RECOMMENDED_MAX_BYTES_OVER_MOBILE,
                        Integer.toString(recommendedMaxBytes));
            }

            // Mobile Data default, based on build
            loadSetting(stmt, Global.MOBILE_DATA,
                    "true".equalsIgnoreCase(
                            SystemProperties.get("ro.com.android.mobiledata",
                                    "true")) ? 1 : 0);

            loadBooleanSetting(stmt, Global.NETSTATS_ENABLED,
                    R.bool.def_netstats_enabled);

            loadBooleanSetting(stmt, Global.USB_MASS_STORAGE_ENABLED,
                    R.bool.def_usb_mass_storage_enabled);

            loadIntegerSetting(stmt, Global.WIFI_MAX_DHCP_RETRY_COUNT,
                    R.integer.def_max_dhcp_retries);

            loadBooleanSetting(stmt, Global.WIFI_DISPLAY_ON,
                    R.bool.def_wifi_display_on);

            loadStringSetting(stmt, Global.LOCK_SOUND,
                    R.string.def_lock_sound);
            loadStringSetting(stmt, Global.UNLOCK_SOUND,
                    R.string.def_unlock_sound);
            loadStringSetting(stmt, Global.TRUSTED_SOUND,
                    R.string.def_trusted_sound);
            loadIntegerSetting(stmt, Global.POWER_SOUNDS_ENABLED,
                    R.integer.def_power_sounds_enabled);
            loadStringSetting(stmt, Global.LOW_BATTERY_SOUND,
                    R.string.def_low_battery_sound);
            loadIntegerSetting(stmt, Global.DOCK_SOUNDS_ENABLED,
                    R.integer.def_dock_sounds_enabled);
            loadIntegerSetting(stmt, Global.DOCK_SOUNDS_ENABLED_WHEN_ACCESSIBILITY,
                    R.integer.def_dock_sounds_enabled_when_accessibility);
            loadStringSetting(stmt, Global.DESK_DOCK_SOUND,
                    R.string.def_desk_dock_sound);
            loadStringSetting(stmt, Global.DESK_UNDOCK_SOUND,
                    R.string.def_desk_undock_sound);
            loadStringSetting(stmt, Global.CAR_DOCK_SOUND,
                    R.string.def_car_dock_sound);
            loadStringSetting(stmt, Global.CAR_UNDOCK_SOUND,
                    R.string.def_car_undock_sound);
            loadStringSetting(stmt, Global.WIRELESS_CHARGING_STARTED_SOUND,
                    R.string.def_wireless_charging_started_sound);

            loadIntegerSetting(stmt, Global.DOCK_AUDIO_MEDIA_ENABLED,
            ...
            loadSetting(stmt, Global.EMERGENCY_TONE, 0);

            // Set default cdma call auto retry
            loadSetting(stmt, Global.CALL_AUTO_RETRY, 0);

            // Set the preferred network mode to target desired value or Default
            // value defined in RILConstants
            int type;
            type = RILConstants.PREFERRED_NETWORK_MODE;
            loadSetting(stmt, Global.PREFERRED_NETWORK_MODE, type);
            ...
            loadSetting(stmt, Global.CDMA_SUBSCRIPTION_MODE, type);

            loadIntegerSetting(stmt, Global.LOW_BATTERY_SOUND_TIMEOUT,
                    R.integer.def_low_battery_sound_timeout);

            loadIntegerSetting(stmt, Global.WIFI_SCAN_ALWAYS_AVAILABLE,
