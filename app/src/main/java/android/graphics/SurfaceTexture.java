 ...
 * {@link Surface} created from a SurfaceTexture can be used as an output
...
public class SurfaceTexture {
     ...
     *
     * @deprecated No longer thrown. {@link Surface.OutOfResourcesException}
     * is used instead.
     */
     ...
     * be used to set the image size when producing images with {@link Canvas} (via
     * {@link Surface#lockCanvas}), or OpenGL ES (via an EGLSurface).
     *
     * The new default buffer size will take effect the next time the image producer requests a
     * buffer to fill.  For {@link Canvas} this will be the next time {@link
     * Surface#lockCanvas} is called.  For OpenGL ES, the EGLSurface should be
     ...
     * calls to {@link #updateTexImage} will throw an {@link IllegalStateException} until
