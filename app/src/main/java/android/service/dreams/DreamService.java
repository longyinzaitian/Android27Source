...
public class DreamService extends Service implements Window.Callback {
    ...
    /**
     * Retrieves the current {@link Window} for the dream.
   ...
   /**
     ...
     * Behaves similarly to {@link android.app.Activity#setContentView(View)} in an activity,
    ...
    /**
     * Controls {@link LayoutParams#FLAG_FULLSCREEN}
    ...
    public void setScreenBright(boolean screenBright) {
        if (mScreenBright != screenBright) {
            ...
            int flag = LayoutParams.FLAG_KEEP_SCREEN_ON;
    ...
    public boolean isScreenBright() {
        return getWindowFlagValue(LayoutParams.FLAG_KEEP_SCREEN_ON, mScreenBright);
    }
     ...
     * {@link PowerManager#PARTIAL_WAKE_LOCK partial wake lock} when necessary.
    ...
    private final void attach(IBinder windowToken, boolean canDoze, IRemoteCallback started) {
        ...
        if (!mWindowless) {
            ...
            if (mDebug) Slog.v(TAG, String.format("Attaching window token: %s to window of type %s",
                    windowToken, LayoutParams.TYPE_DREAM));
            ...
            lp.flags |= ( LayoutParams.FLAG_LAYOUT_IN_SCREEN
                        ...
                        | (mFullscreen ? LayoutParams.FLAG_FULLSCREEN : 0)
                        | (mScreenBright ? LayoutParams.FLAG_KEEP_SCREEN_ON : 0)
                        );
            mWindow.setAttributes(lp);
            // Workaround: Currently low-profile and in-window system bar backgrounds don't go
            // along well. Dreams usually don't need such bars anyways, so disable them by default.
            mWindow.clearFlags(LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
