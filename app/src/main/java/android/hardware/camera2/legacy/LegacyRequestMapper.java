...
public class LegacyRequestMapper {
    ...
    public static void convertRequestMetadata(LegacyRequest legacyRequest) {
        ...
        if (aeFpsRange != null) {
            ...
            if (rangeToApply != null) {
                params.setPreviewFpsRange(rangeToApply[Parameters.PREVIEW_FPS_MIN_INDEX],
                        rangeToApply[Parameters.PREVIEW_FPS_MAX_INDEX]);
            } else {
    ...
    private static <T> T getIfSupported(
            CaptureRequest r, Key<T> key, T defaultValue, boolean isSupported,
            T allowedValue) {
