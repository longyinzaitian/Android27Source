...
public class ViewInfoStoreTest extends TestCase {
    ...
    static class MockViewHolder extends ViewHolder {
        public MockViewHolder() {
            super(new View(null));
        }
    }

    static class MockInfo extends ItemHolderInfo {
    ...
    private ItemHolderInfo find(ViewHolder viewHolder,
            int flags) {
        final int size = mStore.mLayoutHolderMap.size();
        for (int i = 0; i < size; i ++) {
            ViewInfoStore.InfoRecord record = mStore.mLayoutHolderMap.valueAt(i);
            ViewHolder holder = mStore.mLayoutHolderMap.keyAt(i);
